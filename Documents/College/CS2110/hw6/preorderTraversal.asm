...
;CODE
    ; R0: ADDRESS OF ROOT, R1: ADDR OF ARRAY BEGIN, R2: INDEX
    LDR R0, R5, 4 ; R0 <- root
    LDR R1, R5, 5 ; R1 <- arr
    LDR R2, R5, 6 ; R2 <- index
    ; R3: DATA IN ROOT
    ADD R0, R0, 0
    BRnp ENDIF    ; If root == 0
    STR R2, R5, 3
    BRnzp TEARDOWN
    ENDIF
    ...
    ; R3 = ADDR[ROOT] + 1 = ADDR[ROOT.LEFT.ADDRESS]
    ; R3 = ROOT.LEFT.ADDRESS
    LDR R3, R0, #1               ; R3 <- root->left;

    ; PUSH INDEX THEN ARR THEN ROOT.LEFT
    ADD R6, R6, -1
    STR R2, R6, 0
    ADD R6, R6, -1
    STR R1, R6, 0
    ...
    ; INDEX = R2 = PREORDER_TRAVERSAL(ROOT.LEFT, ARR, INDEX)
    LDR R2, R6, 0

    ADD R6, R6, 4

    ; R3 = ROOT.RIGHT.ADDRESS
    LDR R3, R0, 2

    ; PUSH INDEX THEN ARR THEN ROOT.RIGHT
    ADD R6, R6, -1
    STR R2, R6, 0
    ADD R6, R6, -1
    STR R1, R6, 0
    ...
    ; R4 = PREORDER_TRAVERSAL(root.right, arr, index)
    LDR R2, R6, 0
    ADD R6, R6, 4
